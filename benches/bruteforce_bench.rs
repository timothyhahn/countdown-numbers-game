use countdown_numbers_game::BruteForceSolver;
use criterion::{Criterion, criterion_group, criterion_main};
use std::hint::black_box;

fn bench_simple_solution(c: &mut Criterion) {
    c.bench_function("bruteforce simple [10,5] -> 15", |b| {
        b.iter(|| {
...
fn bench_medium_solution(c: &mut Criterion) {
    c.bench_function("bruteforce medium [6,7,7,1,5,8] -> 327", |b| {
        b.iter(|| {
            let mut solver = BruteForceSolver::new();
...
fn bench_complex_solution(c: &mut Criterion) {
    let mut group = c.benchmark_group("bruteforce complex");
...
fn bench_no_solution(c: &mut Criterion) {
    c.bench_function("bruteforce no solution [10,5] -> 3", |b| {
        b.iter(|| {
            let mut solver = BruteForceSolver::new();
...
fn bench_unsolvable_complex(c: &mut Criterion) {
    let mut group = c.benchmark_group("bruteforce unsolvable");
    group.sample_size(10);
